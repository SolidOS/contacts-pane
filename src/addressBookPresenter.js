...
export function renderGroupButtons(currentBook, groupsUl, options, store, domElement, groups_selected, peopleUl, searchEl, cardMainEl, divEl, context, groupClickCallback) {
    kb = store
    dom = domElement
    selectedGroups = groups_selected || {}
...
export function syncGroupUl(book, options, groupsUl, domElement, groups_selected, peopleUl, searchEl) {
    dom = domElement
    if (groups_selected) selectedGroups = groups_selected
    if (peopleUl) ulPeople = peopleUl
    if (searchEl) searchInput = searchEl
    ulGroups = groupsUl
    const groups = groupsInOrder(book, options)
...
function selectPerson(ulPeople, person, detailsView) {
    ...
    kb.fetcher.nowOrWhenFetched(local.doc(), undefined, function (
        ...
        let personHr = dom.createElement('hr')
